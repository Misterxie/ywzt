const Radar = function (radarContainer) {
    radarContainer.innerHTML = '<div class="radar-map-container"> <div class="radar-map"></div></div>' +
                                        ...
                                        '<div class="outer-scanner">' +
                                            '<div class="scanner-container">' +
                                                ...
                                                '<div class="scanner-decoration">' +
                                                    ...
                                                    '<div class="small-ellipse" style="display:none;"></div>' +
                                                    '<div class="small-ellipse" style="display:none;"></div>' +
                                                    '<div class="small-ellipse" style="display:none;"></div>' +
                                                '</div>' +
                                            '</div>' +
    ...
    if( this._container.offsetWidth < this._container.offsetHeight){
        this._containerWidth = this._container.offsetWidth - 10;
        this._containerHeight = this._container.offsetWidth - 10;
    }else{
        this._containerWidth = this._container.offsetHeight - 10;
        this._containerHeight = this._container.offsetHeight - 10;
    }
...
Radar.prototype.addRiskPoints = function(points) {
    ...
    // TODO
    //风险点添加点击操作
    for(var i = 0;i < this._tmpRiskPointsArr.length; i++){
        this._tmpRiskPointsArr[i].index = i;
        this._tmpRiskPointsArr[i].target.addEventListener('click',(e)=>{
            console.log(e)
        })
    }
};
    ...
    elements_group.innerHTML = '<div class="dashed-circle"></div>' +
                                ...
                                '<div class="info-panel">' +
                                    ...
                                    '<div class="info-content">' +
                                        '<div>风险发生地：<span class="risk-city">' + options.city + '</span></div>' +
                                        '<div>风险卡片：<span class="risk-num">' + options.total + '</span></div>' +
                                    '</div>' +
                                '</div>';
...
Radar.prototype._initEvent = function() {
    ...
    // for(var i = 0;i<this._allRiskPointsArr.length; i++ ){

        // this._riskPoints[i].addEventListener('click',(e)=>{
        //     alert('a');
        // },false);
    // }
